...
$(function () {
  ajaxSelProductionNumber();
  makeSummaryTableByOrderSheet();
  makeSummaryTable();
});

function ajaxSelProductionNumber() {
  var fileName = "./php/Export/SelProductionNumber.php";
  var sendData = {
    production_number: $("#production_number").val(),
  };
  myAjax.myAjax(fileName, sendData);
  $("#production_number__select option").remove();
  $("#production_number__select").append($("<option>").val(0).html("NO select"));
  ajaxReturnData.forEach(function(value) {
      $("#production_number__select").append(
...
$(document).on("change", "#export_at", function() {
  var a= $("#export_at").val();
  b=new Date(a)
  c=getDateTime(b)
  console.log(c)
...
function makeSummaryTableByOrderSheet() {
  var fileName = "./php/Export/SelRemain.php";
  var sendData = {
      dummy: "dummy",
  };
  myAjax.myAjax(fileName, sendData);
  fillTableBody(ajaxReturnData, $("#summary__table__remain tbody"));
}

function makeSummaryTable() {
  var fileName = "./php/Export/SelSummary.php";
  var sendData = {
      dummy: "dummy",
  };
  myAjax.myAjax(fileName, sendData);
...
$(document).on("click", "#summary__table tr", function (e) {
  if (!$(this).hasClass("selected-record")) {
    ...
    var fileName = "./php/Export/SelSelExport.php";
    var sendData = {
        id: $("#selected__tr").find("td").eq(0).html(),
    };
    myAjax.myAjax(fileName, sendData);
    console.log(ajaxReturnData);
    $("#production_number__select").val(ajaxReturnData[0].production_number_id);
...
$(document).on("click", "#delete-dialog-delete__button", function () {
  var fileName = "./php/Export/DelSelData.php";
...
$(document).on("click", "#save__button", function() {
  var fileName = "./php/Export/InsExport.php";
  var sendObj = new Object();
...
$(document).on("click", "#update__button", function() {
  var fileName = "./php/Export/UpdateExport.php";
  var sendObj = new Object();
  sendObj["export_at"] =getDateTime(new Date($("#export_at").val()));
  sendObj["production_number__select"] = $("#production_number__select").val();
  sendObj["quantity"] = $("#quantity").val();
