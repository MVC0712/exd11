...
$(document).on("click", "#add__button", function () {
  ...
  $("<td>")
    .append(makeDieNumberSel($("#die-number__select").val()))
    .appendTo(newTr);
  // $("<td>").append(makePressingTypeSel().prop('disabled', true)).appendTo(newTr);
  $("<td>").append(makePressingTypeSel()).appendTo(newTr);
  $("<td>")
    .append($("<input>").attr("type", "text").addClass("code-search"))
    .append(makeCodeSel("").addClass("no-input code-input"))
    .appendTo(newTr);
  // $("<td>").append(makeTime("").addClass("no-input time-input")).appendTo(newTr);
  $("<td>")
    .append(makeInput("").addClass("no-input time-input"))
    .appendTo(newTr);
  $("<td>")
    .append(makeInput("").addClass("no-input time-input"))
    .appendTo(newTr);
...
$(document).on("keyup", "#add__table tbody tr td .code-search", function () {
  ...
  ajaxReturnData.forEach(function (element) {
    $("<option>")
      .html(element["code"])
      .val(element["id"])
      .appendTo("#add__tr td:nth-child(4) select");
  });
});
...
const monthInput = document.getElementById("plan_month");
const startTerm = document.getElementById("plan_start");
const endTerm = document.getElementById("plan_end");

monthInput.addEventListener("change", () => {
  if (!monthInput.value) return;

  // Giá trị monthInput dạng "YYYY-MM"
  ...
  // Format thành yyyy-mm-dd để gán cho input[type=date]
