...
$(document).ready(function () {
  ...
  $("#save__button").on("click", function () {
    ...
    $.ajax({
      url: "./php/Maintenance/InsMaintenanceRecordV2.php",
      type: "POST",
      data: sendObj,
...
function makeSummaryTable() {
  ...
  var sendData = {
    line: $("#line-number-filter").val(),
    machine: $("#machine-number-filter").val(),
    status: $("#status-number-filter").val(),
    start_term: $("#start-term").val(),
    end_term: $("#end-term").val(),
  };

  myAjax.myAjax(fileName, sendData);

  // chia dữ liệu
  const table1Data = ajaxReturnData.filter((row) => row.work_type_id == 1);
  const table2Data = ajaxReturnData.filter((row) => row.work_type_id != 1);

  // clear bảng
  $("#summary__table tbody").empty();
  $("#summary__table1 tbody").empty();

  // fill
  fillTableBody(table1Data, $("#summary__table tbody"));
  fillTableBody(table2Data, $("#summary__table1 tbody"));
}
...
$(document).on(
  "click",
  "#summary__table tbody tr, #summary__table1 tbody tr",
  function () {
    const $currentTable = $(this).closest("table");
    const $otherTable = $currentTable.is("#summary__table")
      ? $("#summary__table1")
      : $("#summary__table");

    if (!$(this).hasClass("selected-record")) {
      // Bỏ chọn dòng cũ trong bảng hiện tại
      $currentTable.find("tbody tr").removeClass("selected-record");

      // Bỏ chọn bảng kia
      $otherTable.find("tbody tr").removeClass("selected-record");

      // Chọn dòng hiện tại
      $(this).addClass("selected-record");

      // Đặt ID cho dòng được chọn
      $currentTable.find("tr#summary__table__selected").removeAttr("id");
      ...
      $.ajax({
        type: "POST",
        url: "./php/Maintenance/SelSelDataV1.php",
...
function fillReadData(record) {
  ...
  $("#work_type").val(record.work_type_id);
...
function fillFileArea(filesBefore, filesAfter) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (file) {
      ...
      if (isImage(file)) {
        container.html(
          `<img src="${basePath}before/${file}" style="width:225px;height:225px;object-fit:contain;border:1px solid #ccc;padding:2px;">`
        );
      } else if (ext === "pdf") {
        container.html(
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (file) {
      ...
      if (isImage(file)) {
        container.html(
          `<img src="${basePath}after/${file}" style="width:225px;height:225px;object-fit:contain;border:1px solid #ccc;padding:2px;">`
        );
      } else if (ext === "pdf") {
        container.html(
...
$(document).on("click", "#update__button", function () {
  let selectedRow = $(
    "#summary__table tbody tr.selected-record, #summary__table1 tbody tr.selected-record"
  );
  ...
  let sendObj = {
    ...
    work_type: $("#work_type").val().trim() || null,
  };

  $.ajax({
    url: "./php/Maintenance/UpdMaintenanceRecordV1.php",
    type: "POST",
    data: sendObj,
    success: function (res) {
...
function setReadonly(selector, readonly) {
  $(selector).prop("readonly", readonly).toggleClass("readonly", readonly);
}

function handleWorkTypeChange() {
  const val = $("#work_type").val();
  const text = $("#work_type option:selected").text();

  if (val == 2 || val == 3 || val == 4 || val == 5 || val == 6) {
    $("#detail").val(text);
    setReadonly("#detail", true);
    setReadonly("#cause", true);
    setReadonly("#re_no", true);
  } else if (val == 1) {
    setReadonly("#detail", false);
    setReadonly("#cause", false);
    setReadonly("#re_no", false);
  }
}

$("#work_type").on("change", handleWorkTypeChange);
handleWorkTypeChange();
