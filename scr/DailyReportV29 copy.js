...
function setSummaryTimeTable() {
  let fileName = "./php/DailyReport/SelSummaryTimeV1.php";
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      data.forEach((row) => {
        ...
        let total_weight = "";
        let productivity = "";
        let actual_cycletime = "";
        if (row.time_start && row.time_end) {
          let diff = diffInMinutes(row.time_start, row.time_end);
          duration = diff + " (min)";
          if (row.Code == "700") {
            total_quantity = Number(row.total_work_quantity) || 0;
            work_weight =
              Number(row.specific_weight) * Number(row.production_length) || 0;
            total_weight = work_weight * total_quantity || 0;
            productivity = total_weight / 1000 / (diff / 60) || 0;
            actual_cycletime = total_quantity
              ? (diff * 60) / total_quantity
              : 0;

            // Làm tròn ngay tại đây
            total_quantity = total_quantity + " (pcs)"; // nếu muốn giữ nguyên, không làm tròn
            work_weight = Number(work_weight.toFixed(3)) + " (kg)";
            total_weight = Number(total_weight.toFixed(2)) + " (kg)";
            productivity = Number(productivity.toFixed(2)) + " (ton/hr)";
            actual_cycletime = Number(actual_cycletime.toFixed(2)) + " (s/pcs)";
          }
        }

        tr.append($("<td>").text(duration));
        tr.append($("<td>").text(total_quantity));
        tr.append($("<td>").text(work_weight));
        tr.append($("<td>").text(total_weight));
        tr.append($("<td>").text(productivity));
        tr.append($("<td>").text(actual_cycletime));
