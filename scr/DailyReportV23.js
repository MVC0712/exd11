...
$(document).on("keydown", "#press-start__input", function (e) {
  if (e.keyCode == 13 && $("#press-start__input").hasClass("complete-input")) {
    const valWithColon = addColon($(this).val());
    $(this).val(valWithColon);
    cancelKeyupEvent = true;
    $("#press-finish__input").focus();

    // Gán giá trị
    $("#Code").val("300");
    $("#time_start").val(valWithColon);
...
$("#add_bundle__button").on("click", function () {
  switch ($(this).text()) {
    ...
    case "Add":
      let fileName;
      let sendData = new Object();
      fileName = "./php/DailyReport/AddBundle15.php";
      sendData = {
        press_id: $("#selected__tr td:nth-child(1)").text(),
...
$(document).on("click", "#summary__table tbody tr", function (e) {
  let fileName = "./php/DailyReport/SelSelData21.php";
  let sendData;
  if (!$(this).hasClass("selected-record")) {
    // tr に class を付与し、選択状態の background colorを付ける
    $(this).parent().find("tr").removeClass("selected-record");
...
$(document).on("change", "#bundle__table tbody tr", function () {
  ...
  let fileName;
  if (editMode) {
    fileName = "./php/DailyReport/UpdateBundleV15.php";
    sendData = {
...
$(document).on("click", "#save__button", function () {
  ...
  fileName = "./php/DailyReport/InsPd21.php";
  sendData = inputData;
  myAjax.myAjax(fileName, sendData);
...
$(document).on("click", "#update__button", function () {
  ...
  // ========Input data============
  inputData = getInputData();
  fileName = "./php/DailyReport/UpdatePd21.php";
  sendData = inputData;
  myAjax.myAjax(fileName, sendData);
...
function updateExtrusionTime() {
  const defaultDuration = getDefaultDuration(); // tổng thời gian mặc định (giây)
  const tableDuration = getTableDuration(); // tổng thời gian từ bảng (giây)

  let result;

  if (tableDuration === 0) {
    const formattedDefault = formatSecondsToHourMinute(defaultDuration);
    $("#tg_dun").text("Extrusion time: " + formattedDefault);
  } else {
    result = defaultDuration - tableDuration;
    if (result < 0) result = 0;
    const formattedResult = formatSecondsToHourMinute(result);
    $("#tg_dun").html("Extrusion time: " + formattedResult);
  }
...
function SumPullTime() {
  ...
  $("#tg_pull").html("Stretching time: " + formattedResult);
  return totalMinutes;
  SumAllTimes();
}
...
function SumCutTime() {
  ...
  // Hiển thị kết quả kèm "minutes"
  const formattedResult = formatSecondsToHourMinute(totalMinutes);
  $("#tg_cut").html("Cutting time: " + formattedResult);
  return totalMinutes;
}

function SumAllTimes() {
  const time1 = updateExtrusionTime(); // thời gian extrusion
  const time2 = SumCutTime(); // thời gian cắt
  const time3 = SumPullTime(); // thời gian kéo

  console.log("time1:", time1);
  console.log("time2:", time2);
  console.log("time3:", time3);

  // Kiểm tra bảng có rỗng hay không
  const isCutEmpty = $("#cut__table tbody tr").length === 0;
  const isPullEmpty = $("#pull__table tbody tr").length === 0;

  if (isCutEmpty || isPullEmpty) {
    // Nếu có ít nhất 1 bảng rỗng, giữ nguyên text mặc định
    $("#total_time").text("Total time");
  } else {
    // Cả 3 bảng đều có dữ liệu, hiển thị tổng
    let total = time1 + time2 + time3;
    const formattedResult = formatSecondsToHourMinute(total);
    $("#total_time").text("Total: " + formattedResult);
  }
...
const machineSelect = document.getElementById("machine-number__select");
const billetSelect = document.getElementById("billet-size__select");

// Lưu trữ các option gốc để reset sau này
const originalOptions = Array.from(billetSelect.options);

machineSelect.addEventListener("change", () => {
  const machineValue = machineSelect.value;

  // Xóa toàn bộ options hiện tại
  billetSelect.innerHTML = "";

  // Luôn thêm option mặc định -
  const defaultOption = originalOptions.find((opt) => opt.value === "0");
  billetSelect.appendChild(defaultOption.cloneNode(true));

  if (machineValue === "1" || machineValue === "3") {
    // Chỉ thêm option 9
    const option9 = originalOptions.find((opt) => opt.value === "9");
    billetSelect.appendChild(option9.cloneNode(true));
  } else if (machineValue === "2") {
    // Thêm option 12 và 14
    const option12 = originalOptions.find((opt) => opt.value === "12");
    const option14 = originalOptions.find((opt) => opt.value === "14");
    billetSelect.appendChild(option12.cloneNode(true));
    billetSelect.appendChild(option14.cloneNode(true));
  } else {
    // Trường hợp khác, thêm tất cả option (ngoại trừ 0 đã thêm)
    originalOptions.forEach((opt) => {
      if (opt.value !== "0") {
        billetSelect.appendChild(opt.cloneNode(true));
      }
    });
  }

  // Reset về option mặc định
  billetSelect.value = "0";
});
