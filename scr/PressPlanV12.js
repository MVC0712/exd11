...
$(document).on("keydown", "#production_number", function (e) {
  if (e.key === "Enter" || e.keyCode === 13) {
    var fileName = "./php/PressPlan/SelProductionNumberV4.php";
    var sendData = {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.success) {
          // Gọi hàm fillTableBody với dữ liệu mảng
          fillTableNitridingBody(response.data, $("#production__table tbody"));
          fillNoPressBody(response.data, $("#production__table tbody"));
        } else {
...
function makeProductionNumber() {
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.success) {
        fillTableBody(response.data, $("#production__table tbody"));
        fillTableNitridingBody(response.data, $("#production__table tbody"));
        fillNoPressBody(response.data, $("#production__table tbody"));
      } else {
        alert("Error: " + (response.error || "Unknown error"));
...
function makeSummaryTable() {
  var fileName = "./php/PressPlan/SelSummaryV9.php";
  var sendData = {
...
function makeOrderBillet() {
  $("#od60612281200").html(
    orderQuantity($("#pl60612281200").html() - $("#ht60612281200").html()),
  );
  $("#od6061228600").html(
    orderQuantity($("#pl6061228600").html() - $("#ht6061228600").html()),
  );
  $("#od60632281200").html(
    orderQuantity($("#pl60632281200").html() - $("#ht60632281200").html()),
  );
  $("#od6063228600").html(
    orderQuantity($("#pl6063228600").html() - $("#ht6063228600").html()),
  );
  $("#od6N012281200").html(
    orderQuantity($("#pl6N012281200").html() - $("#ht6N012281200").html()),
  );
  $("#od6N01228600").html(
    orderQuantity($("#pl6N01228600").html() - $("#ht6N01228600").html()),
  );

  $("#od60613051200").html(
    orderQuantity($("#pl60613051200").html() - $("#ht60613051200").html()),
  );
  $("#od6061305600").html(
    orderQuantity($("#pl6061305600").html() - $("#ht6061305600").html()),
  );
  $("#od60633051200").html(
    orderQuantity($("#pl60633051200").html() - $("#ht60633051200").html()),
  );
  $("#od6063305600").html(
    orderQuantity($("#pl6063305600").html() - $("#ht6063305600").html()),
  );
  $("#od6N013051200").html(
    orderQuantity($("#pl6N013051200").html() - $("#ht6N013051200").html()),
  );
  $("#od6N01305600").html(
    orderQuantity($("#pl6N01305600").html() - $("#ht6N01305600").html()),
  );

  $("#od60613551200").html(
    orderQuantity($("#pl60613551200").html() - $("#ht60613551200").html()),
  );
  $("#od6061355600").html(
    orderQuantity($("#pl6061355600").html() - $("#ht6061355600").html()),
  );
  $("#od60633551200").html(
    orderQuantity($("#pl60633551200").html() - $("#ht60633551200").html()),
  );
  $("#od6063355600").html(
    orderQuantity($("#pl6063355600").html() - $("#ht6063355600").html()),
  );
  $("#od6N013551200").html(
    orderQuantity($("#pl6N013551200").html() - $("#ht6N013551200").html()),
  );
  $("#od6N01355600").html(
    orderQuantity($("#pl6N01355600").html() - $("#ht6N01355600").html()),
  );
}
...
$(document).on("click", "#summary__table tbody tr", function (e) {
  if (!$(this).hasClass("selected-record")) {
    // Xóa trạng thái cũ
    $("#summary__table tbody tr").removeClass(
      "selected-record same-date machine1 machine2 machine3 machine4",
    );
    ...
    // Lấy ngày ở cột thứ 3
    var plDate = $("#selected__tr td:nth-child(3)").text().trim();

    console.log(plDate);

    // Tô màu các dòng cùng ngày theo số máy ở cột thứ 4
    $("#summary__table tbody tr").each(function () {
      var rowDate = $(this).find("td:nth-child(3)").text().trim();
      var machine = $(this).find("td:nth-child(4)").text().trim();
      ...
      if (rowDate === plDate) {
        ...
        switch (machine) {
          case "1":
            $(this).addClass("machine1");
            break;

          case "2":
            $(this).addClass("machine2");
            break;

          case "3":
            $(this).addClass("machine3");
            break;

          case "4":
            $(this).addClass("machine4");
            break;
        }
      }
    });
...
$(document).on("click", "#production__table tbody tr", function (e) {
  if (!$(this).hasClass("selected-record")) {
    ...
    let dies_id = $(this).find("td:nth-child(1)").html();
    let dies = $(this).find("td:nth-child(2)").html();
    let pro_id = $(this).find("td:nth-child(3)").html();
    let pro = $(this).find("td:nth-child(4)").html();
    let max_billet = $(this).find("td:nth-child(14)").html();
    var newTr = $("<tr>");
    var rowCount = $("#add__table tbody tr").length;
    $("<td>").html(dies_id).appendTo(newTr);
    $("<td>").html(pro_id).appendTo(newTr);
    $("<td>").html(pro).appendTo(newTr);
    $("<td>").html(dies).appendTo(newTr);
    $("<td>").append(makeShift()).appendTo(newTr);
    $("<td>")
      .append(makeInput(rowCount + 1))
      .appendTo(newTr);
    $("<td>").append(makeInput("0")).appendTo(newTr);
    $("<td>").append(makeNitride(0)).appendTo(newTr);
    $("<td>").append(makeInput("")).appendTo(newTr);
    $("<td>").html(max_billet).appendTo(newTr);
...
$(document).on("click", "#save__button", function () {
  var fileName = "./php/PressPlan/InsPressPlanV9.php";
...
$(function () {
  $("#print__button").click(function () {
    ...
    if (ajaxReturnData[0].etcing_file_url) {
      var etcing_file_url =
        "../EtchingPicture/" + ajaxReturnData[0].etcing_file_url;
    } else {
      etcing_file_url = "../EtchingPicture/No_image.jpg";
    }
    ...
    var _el = $(
      '<div style="width : 790px; display: flex; flex-direction: row;">',
    );
...
function makeTable(min, max, n) {
  ...
  for (i = min; i < max; ++i) {
    ...
    if (n == 1) {
      if (i % 2 == 0) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n) / (2 * n) + 1) + "H"
        }</td>`;
      } else {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n) / (2 * n) + 1) + "E"
        }</td>`;
      }
    } else if (n == 2) {
      if (i % 4 == 0) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 1) / (2 * n) + 1) + "H"
        }</td>`;
      } else if (i % 4 == 1) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 1) / (2 * n) + 1) + "A"
        }</td>`;
      } else if (i % 4 == 2) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 1) / (2 * n) + 1) + "B"
        }</td>`;
      } else {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 1) / (2 * n) + 1) + "E"
        }</td>`;
      }
    } else if (n == 3) {
      if (i % 6 == 0) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 2) / (2 * n) + 1) + "H"
        }</td>`;
      } else if (i % 6 == 1) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 2) / (2 * n) + 1) + "A"
        }</td>`;
      } else if (i % 6 == 2) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 2) / (2 * n) + 1) + "B"
        }</td>`;
      } else if (i % 6 == 3) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 2) / (2 * n) + 1) + "C"
        }</td>`;
      } else if (i % 6 == 4) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 2) / (2 * n) + 1) + "D"
        }</td>`;
      } else {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 2) / (2 * n) + 1) + "E"
        }</td>`;
      }
    } else if (n == 4) {
      if (i % 8 == 0) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 3) / (2 * n) + 1) + "H"
        }</td>`;
      } else if (i % 8 == 1) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 3) / (2 * n) + 1) + "A"
        }</td>`;
      } else if (i % 8 == 2) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 3) / (2 * n) + 1) + "B"
        }</td>`;
      } else if (i % 8 == 3) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 3) / (2 * n) + 1) + "C"
        }</td>`;
      } else if (i % 8 == 4) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 3) / (2 * n) + 1) + "D"
        }</td>`;
      } else if (i % 8 == 5) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 3) / (2 * n) + 1) + "E"
        }</td>`;
      } else if (i % 8 == 6) {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 3) / (2 * n) + 1) + "F"
        }</td>`;
      } else {
        trDom += `<td style="width: 30px; font-size: 8px;">${
          Math.ceil((i - n - 3) / (2 * n) + 1) + "END"
        }</td>`;
      }
    }
...
function fillTableNitridingBody(data, tbodyDom) {
  let checkLimit = new Object();
  let checkFlag = false;
  $(tbodyDom).empty();
  data.forEach(function (trVal) {
    // Lấy số thực từ after_nitriding_length (bỏ chữ " km")
    let lengthNum = parseFloat(trVal["after_nitriding_length"]);

    checkLimit["length"] = lengthNum;
    checkLimit["die_diamater"] = trVal["die_diamater"];

    if (
      (lengthNum > 3 && trVal["die_diamater"] <= 260) ||
      (lengthNum > 2.5 && trVal["die_diamater"] >= 300) ||
      trVal["washing_count_after_nitriding"] >= 5 ||
      (trVal.die_number &&
        trVal.die_number.startsWith("XLA25") &&
        (Number(trVal.total_profile_length) >= 15 || lengthNum > 2.5)) ||
      (trVal.die_number &&
        trVal.die_number.startsWith("XLA25B2") &&
        lengthNum > 2.5) ||
      (trVal.die_number && /^CP96..T-/.test(trVal.die_number) && lengthNum > 2)
    ) {
      checkFlag = true;
    }
...
function fillNoPressBody(data, tbodyDom) {
  let now = new Date();
  var W1P1 = [];

  // Không xóa tbody nữa, giữ nguyên các <tr> đã có

  // Duyệt từng <tr> hiện có trong tbody
  $(tbodyDom)
    .find("tr")
    .each(function (index, tr) {
      let row = data[index];
      if (!row) return; // tránh lỗi nếu data ít hơn số tr

      // Nếu <tr> đã có class nitriding, bỏ qua
      if ($(tr).hasClass("nitriding")) {
        return;
      }

      let txt_die_id = Number(String(row.die_id).replace(/,/g, ""));
      let txt_pr_tm = Number(String(row.is_washed_die).replace(/,/g, ""));
      let txt_sta_val = Number(String(row.die_status_id).replace(/,/g, ""));
      let txt_d_date = String(row.do_sth_at).replace(/,/g, "");

      let isFixing =
        txt_sta_val === 7 || txt_sta_val === 8 || txt_sta_val === 9;

      let nopressFlag =
        (txt_pr_tm >= 1 && W1P1.includes(txt_die_id)) ||
        (Math.ceil(
          Math.abs(parseDate(txt_d_date) - n_date) / (1000 * 60 * 60 * 24),
        ) >=
          12 * 30 &&
          txt_sta_val == 10 &&
          txt_pr_tm == 0) ||
        txt_pr_tm >= 2 ||
        [3, 31, 32].includes(txt_sta_val);

      if (isFixing) {
        // Gán class fixing cho tất cả td trong tr
        $(tr).find("td").removeClass("nopress").addClass("fixing");
      } else if (nopressFlag) {
        // Gán class nopress cho tất cả td trong tr, xóa fixing nếu có
        $(tr).find("td").removeClass("fixing").addClass("nopress");
      } else {
        // Không thuộc trường hợp nào thì xóa cả hai class
        $(tr).find("td").removeClass("nopress fixing");
      }
    });
}
