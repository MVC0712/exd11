...
$(document).on("keyup change", "#specific_value", function () {
  ...
  if ($(this).attr("type") == "number") {
    ...
    if (a != "") $(this).removeClass("no-input").addClass("complete-input");
    else $(this).removeClass("complete-input").addClass("no-input");
  }
  go_check();
});
...
$(document).on("click", "#summary__table tbody tr", function () {
  ...
  if (!$(this).hasClass("selected-record")) {
    ...
    document.getElementById("sel_die_number").innerHTML =
      ajaxReturnData[0].die_number;
...
$(document).on("click", "#go__button", function () {
  ...
  $("#add__table tbody tr td:nth-child(1)").each(function (index, element) {
  ...
  for (i = 0; i < tr.length; i++) {
    ...
    if (pr_tm) {
      ...
      if (
        ...
        table.rows[i].cells[9].style.backgroundColor = "#bfc1ff";
      } else if (txt_sta_val == 7 || txt_sta_val == 8 || txt_sta_val == 9) {
