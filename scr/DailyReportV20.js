...
$(document).on("keyup", "#len", function() {
  if (
    !isNaN($(this).val()) &&
    $(this).val().length != 0 &&
    300 <= $(this).val() &&
  ...
  switch ($(this).text()) {
    case "Save":
      $("<tr>")
        ...
        .append($("<td>").append($("<input>").val($("#len").val())))
    ...
    case "Add":
      ...
      $("#quantity").val("").removeClass("complete-input").addClass("no-input");
      $("#lot_no").val("").removeClass("complete-input").addClass("no-input");
      $("#len").val("").removeClass("complete-input").addClass("no-input");
      $("#add_bundle__button").prop("disabled", true);
      break;
  }
  if (checkIsDataInputed() && !editMode) {
...
function makeBundleTable() {
  fileName = "./php/DailyReport/SelBundleV20.php";
  sendData = {
    id: $("#selected__tr").find("td").eq(0).html(),
  };
  myAjax.myAjax(fileName, sendData);
...
$(document).on("change", "#bundle__table tbody tr", function () {
  ...
  let fileName;
  if (editMode) {
    fileName = "./php/DailyReport/UpdateBundleV20.php";
    sendData = {
      ...
      len: $("#bundle__selected td:nth-child(5) input").val(),
      mfg: $("#bundle__selected td:nth-child(6) select").val(),
    };
    console.log(sendData);
    myAjax.myAjax(fileName, sendData);
  }
...
$(document).on("click", "#save__button", function () {
  ...
  fileName = "./php/DailyReport/InsBundleV20.php";
...
$(document).on("click", "#update__button", function () {
  ...
  // ========Input data============
  inputData = getInputData();
  fileName = "./php/DailyReport/UpdatePd17.php";
  sendData = inputData;
  myAjax.myAjax(fileName, sendData);
...
function checkBilletQty() {
  ...
  $("#bundle__table tbody tr").each(function () {
    var qty = Number((this).getElementsByTagName("td")[2].getElementsByTagName("input")[0].value);
    var bll = Number((this).getElementsByTagName("td")[4].getElementsByTagName("input")[0].value);
    ttb += qty*bll;
  });
  $("#ttb").html(Math.round(ttb/$("#billet-length__select").val()*10)/10);
  return true;
