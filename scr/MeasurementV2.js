...
let press_id;
let meaStaff = [
  { id: 30, staff_name: "Trần Thị Thảo" },
  { id: 72, staff_name: "Hồ Thị Thủy" },
  { id: 41, staff_name: "Nguyễn Thị Vui" },
  { id: 60, staff_name: "Đinh Kiều Diễm" },
  { id: 82, staff_name: "Trần Thị Mỹ Hà" },
  { id: 83, staff_name: "Nguyễn Thị Diễm Trinh" },
];
...
$(document).on("click", "#summary_table tbody tr", function (e) {
  if (!$(this).hasClass("selected-record")) {
    ...
    $("#data__table tbody tr").remove();
    let totalRows =
      n == 1 || n == 2 ? Math.ceil(a * 3 * m * h) : Math.ceil(a * 2 * m * h);
    ...
    for (i = 0; i < totalRows; ++i) {
      ...
      if (n == 1) {
        if (i % 3 == 0) {
          trDom.append(
            $("<td>").html(Math.ceil((i - n - 1) / (3 * n) + 1) + "H"),
          );
        } else if (i % 3 == 1) {
          trDom.append(
            $("<td>").html(Math.ceil((i - n - 1) / (3 * n) + 1) + "M"),
          );
        } else {
          trDom.append(
            $("<td>").html(Math.ceil((i - n - 1) / (3 * n) + 1) + "E"),
          );
        }
      } else if (n == 2) {
        if (i % 6 == 0) {
          trDom.append(
            $("<td>").html(Math.ceil((i - n - 3) / (3 * n) + 1) + "H"),
          );
        } else if (i % 6 == 1) {
          trDom.append(
            $("<td>").html(Math.ceil((i - n - 3) / (3 * n) + 1) + "M"),
          );
        } else if (i % 6 == 2) {
          trDom.append(
            $("<td>").html(Math.ceil((i - n - 3) / (3 * n) + 1) + "A"),
          );
        } else if (i % 6 == 3) {
          trDom.append(
            $("<td>").html(Math.ceil((i - n - 3) / (3 * n) + 1) + "B"),
          );
        } else if (i % 6 == 4) {
          trDom.append(
            $("<td>").html(Math.ceil((i - n - 3) / (3 * n) + 1) + "M1"),
          );
        } else {
          trDom.append(
            $("<td>").html(Math.ceil((i - n - 3) / (3 * n) + 1) + "E"),
          );
        }
      } else if (n == 3) {
        if (i % 6 == 0) {
...
function selectRow(row) {
  ...
  if (!$row.hasClass("selected-record")) {
    $row.parent().find("tr").removeClass("selected-record");
    $row.addClass("selected-record");

    $("#data_select_tr").removeAttr("id");
    $row.attr("id", "data_select_tr");
  }
}

$(document).on("click", "#data__table tbody tr", function () {
  selectRow(this);
});
