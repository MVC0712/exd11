...
function makeSummaryTableTotalImport() {
  $.ajax({
    url: "./php/Import/SelSummaryTotalImport.php",
    type: "POST",
    data: {},
    success: function (response) {
      ...
      const tbody = $("#summary_import__table tbody").empty();

      // tách 2 nhóm: dòng cảnh báo và bình thường
      const warningRows = [];
      const normalRows = [];

      data.forEach((row) => {
        // thêm % cho cột cuối
        row.import_percentage = row.import_percentage + "%";

        // tách theo điều kiện >100%
        const percentValue = parseFloat(row.import_percentage.replace("%", ""));
        if (percentValue > 100) {
      ...
      const appendRows = (rows) => {
        rows.forEach((row) => {
          ...
          const percentValue = parseFloat(
            row.import_percentage.replace("%", ""),
          );
          if (percentValue > 100) tr.addClass("arlam"); // tô đỏ
          Object.values(row).forEach((val) => $("<td>").text(val).appendTo(tr));
          tr.appendTo(tbody);
        });
      };

      // append dòng cảnh báo trước
      appendRows(warningRows);
      // append các dòng còn lại
