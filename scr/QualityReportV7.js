...
function fillPressData() {
  ...
  var qcResult = ajaxReturnData[0]["qc_check_result"];
  $("#qc_check_show").val(qcResult).removeClass("qc-ok qc-ng");

  if (qcResult === "Đã kiểm tra") {
    $("#qc_check_show").addClass("qc-ok");
  } else if (qcResult === "NG") {
    $("#qc_check_show").addClass("qc-ng");
  }
...
$(document).on("click", "#save__button", function () {
  const fileName = "./php/QualityReport/UpdateQC.php";
  const sendData = {
    process_id: $('input:radio[name="process_name"]:checked').val(),
    press_id: press_id, // biến global
    qc_check_id: $("#qc__check").val(),
    qc_name_id: $("#qc__select").val(), // đúng id select QC
  };

  // Gọi Ajax
  myAjax.myAjax(fileName, sendData);
  $("#qc__check").val(0).removeClass("complete-input").addClass("no-input");
...
$(document).on("change", "#qc__check", function () {
  if ($(this).val() != 0) {
    $(this).removeClass("no-input").addClass("complete-input");
  } else {
    $(this).removeClass("complete-input").addClass("no-input");
