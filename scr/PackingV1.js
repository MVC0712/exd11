...
function setRecentryPackingWork() {
  ...
  fileName = "./php/Packing/SelPackingHistoryAllV1.php";
  sendData = {
...
$(document).on("blur", "#packing-start__input", function () {
...
$(document).on("blur", "#packing-end__input", function () {
  if (checkTimeValue($(this).val())) {
    $(this).val(addColon($(this).val()));
  }
});
...
$("#download_data").on("click", function () {
  let start = $("#start-term").val();
  let end = $("#end-term").val();

  let url =
    "./php/Packing/DownloadSummary.php?start_date=" +
    encodeURIComponent(start) +
    "&end_date=" +
    encodeURIComponent(end);

  // redirect để download file
  window.location.href = url;
});

const monthInput = document.getElementById("month");
const startTerm = document.getElementById("start-term");
const endTerm = document.getElementById("end-term");
