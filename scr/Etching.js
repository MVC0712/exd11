...
function makeDieOption() {
    var fileName = "./php/Etching/SelDieNumber.php";
    var sendData = {
...
$(document).on("change", "#date__input", function() {
    ...
    $("#die_number").addClass("no-input").removeClass("complete-input");
...
$(document).on("change", "#die_number", function() {
    $("#etching__date").val("").addClass("no-input").removeClass("complete-input");
    if ($(this).val() != 0) {
...
// console.log($('#data__table tbody td').map(function(){
//     return $(this).find("select").find("option:selected").text().split(/\W+/).filter(function(v){return v=="311"}).length
//         }).toArray().reduce(function(p,v){return p+v}));
