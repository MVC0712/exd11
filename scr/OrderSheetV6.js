...
function readSummaryTable() {
  ...
  ajaxReturnData.forEach(function (trVal) {
    ...
    $("<td>")
      ...
      .html(`<input type="checkbox" class="delete-check">`)
      .appendTo(newTr);
...
$(document).on("click", "#delete__button", function () {
  ...
  $("#summary__table tbody tr").each(function () {
    ...
    const checkbox = tr.find(".delete-check");
  ...
  $.ajax({
    url: "./php/OrderSheet/DeleteOrdersheet.php",
    type: "POST",
    dataType: "json",
    data: {
      id: selectedIds[0], // Assuming you want to delete one at a time
      password: password,
    },
...
function toggleDeleteButton() {
  const anyChecked = $(".delete-check:checked").length > 0;

  $("#delete__button").prop("disabled", !anyChecked);
}
$(document).on("change", ".delete-check", function () {
  updateSelectedList();
});
function updateSelectedList() {
  const tbody = $("#selected_table tbody");
  tbody.empty();
  ...
  $(".delete-check:checked").each(function () {
    ...
    const ordersheetNumber = tr.find("td:eq(1)").text();
