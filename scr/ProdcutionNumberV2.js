...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//  save button activation condition
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
function checkInputComplete() {
  let flag = true;
  if ($("#production_number").val().length < 3) flag = false;

  $(".input__area option:selected").each(function () {
    if ($(this).val() == 0) flag = false;
  });

  $(".input__area input:not(:eq(0))").each(function () {
    if ($(this).val().length == 0 || isNaN($(this).val())) flag = false;
  });

  if ($("#production_length").val() < 1.5 || 8 < $("#production_length").val())
    flag = false;

  if (
    $("#circumscribed_circle").val() < 0 ||
    2000 < $("#circumscribed_circle").val()
  )
    flag = false;

  if ($("#specific_weight").val() < 0 || 50 < $("#specific_weight").val())
    flag = false;

  if (
    $("#cross_section_area").val() < 0 ||
    5000 < $("#cross_section_area").val()
  )
    flag = false;

  if ($("#packing_quantity").val() < 0 || 1000 < $("#packing_quantity").val())
    flag = false;
  if ($("#packing_row").val() < 0 || 200 < $("#packing_row").val())
    flag = false;
  if ($("#packing_column").val() < 0 || 200 < $("#packing_column").val())
    flag = false;
  return flag;
}
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
$(document).on("keyup", ".main__wrapper", function () {
