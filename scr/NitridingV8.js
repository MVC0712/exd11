...
function makeSummaryTable() {
  var fileName = "./php/Nitriding/SelSummaryV6.php";
  var sendData = {
    dummy: "dummy",
  };
  // 今日の日付の代入
  ...
  data.forEach(function (trVal) {
    ...
    if (
      ...
      (trVal.die_number &&
        ...
        (Number(trVal.total_profile_length) >= 15 ||
          trVal["after_nitriding_length"] > 2.5)) ||
      (trVal.die_number &&
        trVal.die_number.startsWith("XLA25B2") &&
        trVal["after_nitriding_length"] > 2.5) ||
      (trVal.die_number &&
        /^CP96..T-/.test(trVal.die_number) &&
        trVal["after_nitriding_length"] > 2)
    ) {
      chekFlag = true;
    }
    let newTr = $("<tr>");
...
$(document).on("click", "#nitriding__button", function () {
  ...
  $("#nitriding__table tbody tr td:nth-child(1)").each(
