...
$("#add_pull__button").on("click", function () {
  switch ($(this).text()) {
    case "Save":
      $("<tr>")
        .append("<td></td>")
        .append($("<td>").append($("<input>").val($("#pull_date").val())))
        .append($("<td>").append($("<input>").val($("#pull_no1").val())))
        .append($("<td>").append($("<input>").val($("#pull_no2").val())))
        .append($("<td>").append($("<input>").val($("#pull_start").val())))
        .append($("<td>").append($("<input>").val($("#pull_end").val())))
        .appendTo("#pull__table tbody");
      SumPullTime();
      $(this).prop("disabled", true);
      $("#pull_date")
        .val("")
...
$("#add_cut__button").on("click", function () {
  switch ($(this).text()) {
    case "Save":
      $("<tr>")
        .append("<td></td>")
        .append($("<td>").append($("<input>").val($("#cut_date").val())))
        .append($("<td>").append($("<input>").val($("#cut_no1").val())))
        .append($("<td>").append($("<input>").val($("#cut_no2").val())))
        .append($("<td>").append($("<input>").val($("#cut_start").val())))
        .append($("<td>").append($("<input>").val($("#cut_end").val())))
        .appendTo("#cut__table tbody");
      SumCutTime();
      $(this).prop("disabled", true);
      $("#cut_date").val("").removeClass("complete-input").addClass("no-input");
...
function updateExtrusionTime() {
  ...
  const tableDuration = getTableDuration();

  let result;

  if (tableDuration === 0) {
    $("#tg_dun").text("Extrusion time: " + defaultDuration + " seconds");
  } else {
    result = defaultDuration - tableDuration;
    if (result < 0) result = 0;
    $("#tg_dun").html("Extrusion time: " + result + " seconds");
  }
...
function SumPullTime() {
  ...
  $("#tg_pull").html("Stretching time: " + totalMinutes + " seconds");
  return totalMinutes;
  SumAllTimes();
}
...
function SumCutTime() {
  ...
  // Hiển thị kết quả kèm "minutes"
  $("#tg_cut").html("Cutting time: " + totalMinutes + " seconds");
  return totalMinutes;
}
...
function SumAllTimes() {
  ...
  if (isCutEmpty || isPullEmpty) {
    ...
    $("#total_time").text("Total: " + total + " seconds");
  }
