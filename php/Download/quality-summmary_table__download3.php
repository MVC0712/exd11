...
// 21/06/18
  ...
  try {
      ...
      $export_csv_title = [
        ...
        "停机痕不良", "卷裂不良", "其他不良", "製造", "生産性", "総時間", "300コード時間", "200コード時間"
      ];
      $export_csv_title = [
        ...
        "code_321", "code_322", "code_323", "code_324", "code_351", "code_401", "machine", "mfg", "productivity", "total_time", "time code 300", "time code 200"];
      $export_sql = "
			SELECT 
    ...
    ,2)
END AS total_time,
	IFNULL(ROUND(t300.time_300, 4), 0) as time_code_300,
	IFNULL(ROUND(t200.time_200, 4), 0) as time_code_200
		FROM t_press
		...
		LEFT JOIN t_bundle ON t_press.id = t_bundle.press_id
		LEFT JOIN t_time_press ON t_press.id = t_time_press.press_id
		LEFT JOIN 
			(
				SELECT 
		...
		LEFT JOIN (
    			...
    			SUM(
      			CASE 
        			WHEN Code = 300
        			THEN ((TIME_TO_SEC(time_end) - TIME_TO_SEC(time_start))/3600)
        			ELSE 0
      			END
    			) AS time_300
  			FROM t_time_press
  			GROUP BY press_id
			) t300 ON t300.press_id = t_press.id
		LEFT JOIN (
  				SELECT 
    			press_id,
    			SUM(
  			...
  			FROM t_time_press
  			GROUP BY press_id
			) t200 ON t200.press_id = t_press.id
 		LEFT JOIN (
  			SELECT 
    			press_id,
