...
try {
        ...
        SELECT 
            ...
            ROUND(
                COALESCE(
                    (
                        SELECT
                            SUM(TIMESTAMPDIFF(MINUTE, tt.time_start, tt.time_end)) / 60
                            + t_maintenance_record.repair_time
                        FROM t_maintenance_time tt
                        WHERE tt.maintenance_record_id = t_maintenance_record.id
                    ),
                    t_maintenance_record.repair_time
                ),
                2
    ...
    $sql .= " ORDER BY FIELD(t_maintenance_record.equip_status_id, 0, 2, 3, 4, 1), t_maintenance_record.date_occur DESC";
