...
if ($start_date == '' && $end_date == '') {
    // tháng hiện tại
    $where = "t_packing.packing_date BETWEEN DATE_FORMAT(CURDATE(), '%Y-%m-01') AND CURDATE()";
}
elseif ($start_date == '' && $end_date != '') {
    // từ đầu tháng của end → end
    $where = "t_packing.packing_date BETWEEN DATE_FORMAT(:end_date, '%Y-%m-01') AND :end_date";
}
elseif ($start_date != '' && $end_date == '') {
    // từ start → hôm nay
    $where = "t_packing.packing_date BETWEEN :start_date AND CURDATE()";
}
else {
    // start → end
    $where = "t_packing.packing_date BETWEEN :start_date AND :end_date";
}
  try {
      $dbh = new PDO(
...
if ($start_date != '') {
    $prepare->bindValue(':start_date', $start_date, PDO::PARAM_STR);
}

if ($end_date != '') {
    $prepare->bindValue(':end_date', $end_date, PDO::PARAM_STR);
}
